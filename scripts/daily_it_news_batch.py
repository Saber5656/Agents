    ...
    for attempt in range(3):
        try:
            ...
            if exc.errno != errno.EDEADLK:
                raise
            content = b''
        if content and hashlib.sha256(content).hexdigest() == expected_sha256:
            ...
            write_json(run_root/'advisory-input.json', {'name': summary.name, 'sha256': expected_sha256,
                       'bytes': len(content), 'snapshot_path': str(path), 'read_attempts': attempt + 1})
            return path
        if attempt < 2:
            time.sleep(0.5)
    raise news.RunnerError('cannot read content matching the verified summary digest')
...
def generate_advisory(config, summary, run_root, expected_summary_sha256):
    ...
    try:
        ...
        if path.is_file() and not path.is_symlink() and news.digest(path)==data['sha256']:
            return path
...
'''
    ...
    news.logged_command(subprocess.run,command,run_root,'advisory',cwd=stage,
    ...
    for i in range(1,1000):
        ...
        try:
            ...
            path.unlink();raise
        write_json(cached,{'path':str(path),'sha256':news.digest(path),'summary_sha256':expected_summary_sha256})
        return path
    raise news.RunnerError('advisory filename collision limit')


def complete_delivery(config, env, summary, advisory, run_root):
    import daily_it_news_delivery as delivery
    required=['USER_VAULT_ROOT','USER_VAULT_REMOTE','AGENTS_VAULT_REMOTE','NEWS_PUBLICATION_BRANCH',
    ...
    def publish(remote, artifact, relative, phase):
        return delivery.publish_artifact(remote,env['NEWS_PUBLICATION_BRANCH'],artifact,relative,
                 run_root/phase,env['PUBLISHER_GIT_NAME'],env['PUBLISHER_GIT_EMAIL'],env['GITLEAKS_BIN'])
    # A reviewed optional candidates file must exist in the news commit if linked.
    for name in re.findall(r'\]\((IT-NEWS-SOURCES-\d{4}-\d{2}-\d{2}\.json)\)',summary.read_text()):
        sidecar=summary.parent/name
        publish(env['USER_VAULT_REMOTE'],sidecar,sidecar.relative_to(user_root).as_posix(),'publish-sources')
    public_news=publish(env['USER_VAULT_REMOTE'],summary,summary_relative,'publish-news')
    public_advice=publish(env['AGENTS_VAULT_REMOTE'],advisory,advisory_relative,'publish-advisory')
    write_json(run_root/'publication.json',{'news':public_news,'advisory':public_advice})
    key=hashlib.sha256((news.digest(summary)+news.digest(advisory)).encode()).hexdigest()
    result=delivery.notify_discord(str(news.dt.datetime.now(news.JST).date()),public_news['url'],public_advice['url'],key,
...
def run_batch(config, env, *, force=False, resume_run=None):
    ...
    state={'status':'blocked','phase':'news','started_at':now.isoformat(timespec='seconds'),'run_root':str(run_root)}
    try:
        run_root.mkdir(parents=True)
        result=news.run(config,force=force,resume_run=resume_run)
        state['news_result']=result
        if result['status'] not in {'complete','already_complete'}:raise news.RunnerError('news stage failed: '+str(result.get('error','')))
        summary=Path(result['summary_path']);state['summary_path']=str(summary)
        state['phase']='advisory';write_json(run_root/'status.json',state)
        advisory=generate_advisory(config,summary,run_root,result['summary_sha256']);state['advisory_path']=str(advisory)
        state['phase']='publication';write_json(run_root/'status.json',state)
        sent=complete_delivery(config,env,summary,advisory,run_root)
        state.update(phase='discord',delivery=sent)
        if sent['delivery_status'] not in {'delivered','skipped'}:raise news.RunnerError('Discord delivery is not confirmed')
