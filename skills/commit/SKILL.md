...
5. Stage explicit paths or hunks (`scripts/stage_approved_patch.py` can assist after inspecting its current interface). Inspect `git diff --cached` and ensure it is the reviewed change. Never use a whole-tree add for mixed work. When a task hunk and another purpose are mixed in one file, initialize an alternate index from `HEAD`, apply the reviewed patch with `--index-file`, and run `git commit` with the same `GIT_INDEX_FILE`; this keeps the primary index and worktree untouched.
