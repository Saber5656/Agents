...
For a new task, use `harness.delivery.prepare_worktree` or equivalent `git worktree add -b` at the selected SHA. Verify actual cwd, branch and HEAD before starting the writer. For a resume, reuse the matching task/path/branch, verify ancestry and ownership of changes since base, and retain issue-owned commits and dirty work. Do not require a resumed HEAD to equal the original base. A pre-existing branch without the matching worktree is `reconciliation_required`; a detached/unrelated path is `workspace_conflict`; an unavailable base is `base_unavailable`; and remote fetch/readback failure is `remote_incomplete`. These states preserve the checkout and do not trigger reset, stash, clean, or delete.
