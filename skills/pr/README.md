...
checks commit messages and diffs before transmission, `GitHub.push_branch` requires the immutable
reviewed `base` and explicit `expected_remote` before performing a non-force push with exact
branch/commit and remote readback, and `GitHub.create_or_reuse_pr` requires the reviewed `head_oid`
while searching the exact head/base pair before creating and reconciling a lost response. An empty
or stale listing never authorizes a second create. These helpers do not claim GitHub exactly-once
semantics; an uncertain external result remains pending until the same remote object is observed.
