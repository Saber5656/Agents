...
3. Record material choices in the Decision Ledger; when a requirement changes, retain the prior statement and rationale, add the newly accepted scope as a linked entry, and route unresolved product/architecture/security choices to the declared `caller | user` owner.
