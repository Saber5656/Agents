...
3. Record material choices in the Decision Ledger; when a requirement changes, retain the prior statement and rationale when supplied, add the newly accepted scope as a linked entry, and route unresolved material product/architecture/security choices to the declared `caller | user` owner. Mark an unavailable prior rationale as unknown; an explanation for archiving the old entry is not its original product rationale. Requirements already explicit in the input stay accepted. Assign ordinary schema and implementation details to the implementing agent rather than requesting caller confirmation.
